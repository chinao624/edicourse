...
<body class="bg-gradient-to-br from-gray-100 to-purple-100">
    ...
    @if (session('success'))
    <div class="alert alert-success bg-[#5f9ea0]">
        {{ session('success') }}
    </div>
@endif
    <div class="min-h-screen flex items-center justify-center">
        <div class="bg-white p-10 rounded-lg shadow-lg w-full max-w-md">
            <h1 class="text-3xl font-light text-gray-700 mb-8 text-center">Reviewer Login</h1>
            @if($errors->has('error'))
                <div class="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 mb-6" role="alert">
                    <p>{{ $errors->first('error') }}</p>
                </div>
            @endif
            <form action="{{ route('reviewer.login') }}" method="POST">
                @csrf
                <div class="mb-6">
                ...
                <div class="flex items-center justify-between mb-6">
                    ...
                    <a href="{{ route('reviewer.create') }}" class="text-sm text-purple-600 hover:text-purple-800 transition duration-300">アカウント登録</a>
                </div>
            </form>
        </div>
    </div>
